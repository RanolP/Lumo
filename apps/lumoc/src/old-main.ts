        ...
        dsl.c.bind(
          ...
          (g) =>
            dsl.c.bind(
              ...
              (res) => res.produce().annotate(A.comput()),
            ),
        ),
      )
      .thunk(),
      ...
      .lambda((x) =>
        Computation.Match(Value.Unroll(x), {
          [nat.zero.tag]: dsl.f.matchArm((x) =>
            maybe_nat.nothing.v.produce().annotate(maybe_nat.t.comput()),
          ),
          [nat.succ.tag]: dsl.f.matchArm((x) =>
            dsl.c.bind(x.select('0'), (y) =>
              maybe_nat.just.v(y).produce().annotate(maybe_nat.t.comput()),
            ),
          ),
        }),
      )
              ...
              Computation.With({
                  ...
                  .lambda((v) =>
                    Computation.Match(v.unroll(), {
                      [nat.zero.tag]: dsl.f.matchArm((v) =>
                        bool.true.v.produce().annotate(dsl.t.var().comput()),
                      ),
                ...
                isOdd: dsl.c
                  .lambda((v) =>
                    Computation.Match(v.unroll(), {
                      [nat.zero.tag]: dsl.f.matchArm((v) =>
                        bool.false.v.produce().annotate(dsl.t.var().comput()),
                      ),
                      [nat.succ.tag]: dsl.f.matchArm((v) =>
                        dsl.c.bind(v.select('0'), (n) =>
                          Computation.Apply(
