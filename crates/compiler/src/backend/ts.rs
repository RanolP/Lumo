...
impl TypeScriptBackend {
    ...
    fn lower_file(&self, file: &lir_memaware::File) -> Result<tsast::Program, BackendError> {
        ...
        tsast::inline_literal_consts(&mut program);
        tsast::simplify_bool_comparisons(&mut program);
...
fn type_refs_self(ty: &TypeExpr) -> bool {
    match ty {
        ...
        TypeExpr::Proj { base, .. } => type_refs_self(base),
...
fn lower_type_expr_to_ts_type(ty: &TypeExpr) -> tsast::TsType {
    match ty {
        ...
        TypeExpr::Proj { base, assoc } => {
            tsast::TsType::TypeRef(format!("{}[\"{assoc}\"]", type_expr_to_ts_text(base)))
        }
...
fn type_expr_to_ts_text(ty: &TypeExpr) -> String {
    match ty {
        ...
        TypeExpr::Proj { base, assoc } => format!("{}[\"{assoc}\"]", type_expr_to_ts_text(base)),
