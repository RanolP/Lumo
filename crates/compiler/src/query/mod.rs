...
impl QueryEngine {
    ...
    pub fn lower_module(&mut self, files: &[&str]) -> Option<lir::File> {
        ...
        // Phase 3: Resolve default cap impls (Perform → Ident)
        resolve_default_cap_impls(&mut lowered);
        // Phase 4: Re-run cap inference on patched LIR
...
/// Resolve default cap impls: replace `Perform` nodes with `Ident` references
/// to the corresponding impl const when a default implementation is available.
///
/// Two kinds of default impl:
/// - **Platform**: inherent impl whose target name matches a cap name
///   (e.g. cap `StrOps` + `impl StrOps { ... }` → const `StrOps`)
/// - **Typeclass**: `impl Type: Cap { ... }` → default for `Cap[Type]`
///   (e.g. `impl Number: Add { ... }` → default for `Add[Number]`, const `__impl_Number_Add`)
fn resolve_default_cap_impls(file: &mut lir::File) {
    // Collect cap names
    let cap_names: HashSet<String> = file
        .items
        .iter()
        .filter_map(|item| {
            if let lir::Item::Cap(c) = item {
                Some(c.name.clone())
            } else {
                None
            }
    ...
    // Build default impl map: (cap_name, type_args) → impl const name
    let mut default_impls: HashMap<(String, Vec<String>), String> = HashMap::new();

    for item in &file.items {
        if let lir::Item::Impl(impl_decl) = item {
            let target = impl_decl.target_type.value.display();
            if impl_decl.capability.is_none() && cap_names.contains(&target) {
                // Platform default impl: inherent impl whose target = cap name
                // e.g. impl StrOps { ... } → cap ("StrOps", ["StrOps"]), const "StrOps"
                default_impls.insert((target.clone(), vec![target.clone()]), target);
            } else if let Some(cap_ty) = &impl_decl.capability {
                let cap = cap_ty.value.display();
                if cap_names.contains(&cap) {
                    // Typeclass default impl: impl Type: Cap { ... }
                    // e.g. impl Number: Add { ... } → cap ("Add", ["Number"]), const "__impl_Number_Add"
                    let const_name = if let Some(name) = &impl_decl.name {
                        name.clone()
                    } else {
                        format!("__impl_{target}_{cap}")
                    };
                    default_impls.insert((cap, vec![target]), const_name);
                }
            }
        }
    }

    if default_impls.is_empty() {
        return;
    }

    // Walk all function and impl method bodies, replacing resolved Performs with Idents
    for item in &mut file.items {
        match item {
            lir::Item::Fn(f) => {
                resolve_expr_default_caps(&mut f.value, &default_impls, &HashSet::new());
            }
            lir::Item::Impl(impl_decl) => {
                for m in &mut impl_decl.methods {
                    resolve_expr_default_caps(&mut m.value, &default_impls, &HashSet::new());
                }
            }
            _ => {}
        }
...
/// Walk an expression tree, replacing `Perform { cap, type_args }` with `Ident { name: impl_const }`
/// when the cap has a default impl and is not inside an explicit `Handle` for the same cap+type_args.
fn resolve_expr_default_caps(
    expr: &mut lir::Expr,
    default_impls: &HashMap<(String, Vec<String>), String>,
    handled: &HashSet<(String, Vec<String>)>,
) {
    match expr {
        lir::Expr::Perform { id, cap, type_args } => {
            let key = (cap.clone(), type_args.clone());
            if !handled.contains(&key) {
                if let Some(const_name) = default_impls.get(&key) {
                    *expr = lir::Expr::Ident {
                        id: *id,
        ...
        lir::Expr::Handle {
            ...
            // Handler is resolved in the outer scope
            resolve_expr_default_caps(handler, default_impls, handled);
            // Body has this cap explicitly handled — don't auto-resolve it
            let mut inner_handled = handled.clone();
            inner_handled.insert((cap.clone(), type_args.clone()));
            resolve_expr_default_caps(body, default_impls, &inner_handled);
        }
        lir::Expr::Apply { callee, arg, .. } => {
            resolve_expr_default_caps(callee, default_impls, handled);
            resolve_expr_default_caps(arg, default_impls, handled);
        }
        lir::Expr::Let { value, body, .. } => {
            resolve_expr_default_caps(value, default_impls, handled);
            resolve_expr_default_caps(body, default_impls, handled);
        }
        lir::Expr::Match { scrutinee, arms, .. } => {
            resolve_expr_default_caps(scrutinee, default_impls, handled);
            for arm in arms {
                resolve_expr_default_caps(&mut arm.body, default_impls, handled);
            }
        }
        lir::Expr::Lambda { body, .. } => resolve_expr_default_caps(body, default_impls, handled),
        lir::Expr::Thunk { expr, .. }
        | lir::Expr::Produce { expr, .. }
        | lir::Expr::Force { expr, .. }
        | lir::Expr::Unroll { expr, .. }
        | lir::Expr::Roll { expr, .. }
        | lir::Expr::Ann { expr, .. } => resolve_expr_default_caps(expr, default_impls, handled),
        lir::Expr::Member { object, .. } => {
            resolve_expr_default_caps(object, default_impls, handled)
        }
        lir::Expr::Bundle { entries, .. } => {
            for e in entries {
                resolve_expr_default_caps(&mut e.body, default_impls, handled);
            }
        }
        lir::Expr::Ctor { args, .. } => {
            for a in args {
                resolve_expr_default_caps(a, default_impls, handled);
            }
        }
        lir::Expr::Ident { .. }
        | lir::Expr::String { .. }
