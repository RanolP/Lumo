...
pub struct ExprBody<'a>(pub(crate) &'a SyntaxNode);
...
impl<'a> CapAnnotation<'a> {
    pub fn cap(&self) -> Option<CapSig<'a>> {
        self.0.children.iter().find_map(|c| match c {
            SyntaxElement::Node(n) => CapSig::cast(n),
            _ => None,
        })
    }
}

pub struct CapSig<'a>(pub(crate) &'a SyntaxNode);
...
pub struct IfElseExpr<'a>(pub(crate) &'a SyntaxNode);
